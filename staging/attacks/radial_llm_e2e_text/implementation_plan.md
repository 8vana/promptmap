# Implementation Plan: Radial LLM E2E Text Attack

## Summary

- `attack_id`: `radial_llm_e2e_text`
...
RADIAL is an automated jailbreak method that analyzes the inherent response tendency of LLMs by calculating generation probabilities for affirmative vs. rejection responses to real-world instructions. It identifies instructions that inherently induce affirmative responses, then strategically splices these around malicious instructions to bypass safety mechanisms. The method requires only 40 manually-crafted responses (20 affirmative, 20 rejection) and produces semantically coherent attack prompts.

## Algorithm Steps

### 1. Construct Response Templates

Create 20 general affirmation responses (e.g., 'Sure, here's the information.') and 20 general rejection responses (e.g., 'Sorry, I am unable to provide the information') that are not specific to any particular instruction.

- `status`: `supported`
- `evidence_refs`: paper:section:3_2, paper:appendix_ref:a

### 2. Collect Real-World Instructions

Gather 30,000 real-world English instructions from the alpaca official repository for analysis.

- `status`: `supported`
- `evidence_refs`: paper:section:3_2, paper:footnote:1

### 3. Calculate Response Probabilities

For each real-world instruction X, calculate the probability of generating affirmation response y_a and rejection response y_r using the formulas provided in the paper.

- `status`: `ambiguous`
- `evidence_refs`: paper:section:3_2, paper:formula:3_2

### 4. Compute Inherent Response Tendency Score

Calculate a score for each instruction reflecting the LLM's inherent tendency to affirm, with higher scores indicating stronger affirmation tendency.

- `status`: `ambiguous`
- `evidence_refs`: paper:section:3_2

### 5. Rank Instructions by Affirmation Tendency

Create a ranking of real-world instructions based on their calculated scores, with highest-scoring (most affirmation-inducing) instructions at the top.

- `status`: `supported`
- `evidence_refs`: paper:section:3_2, paper:figure:2

### 6. Filter Instruction Types

Remove text manipulation instructions (translation, rewriting, etc.) that would cause the LLM to manipulate the malicious instruction rather than execute it.

- `status`: `supported`
- `evidence_refs`: paper:section:3_3

### 7. Construct Attack Prompt

Select top-ranked real-world instructions and strategically splice them around the malicious instruction, using 2 or 4 instructions total with the malicious instruction positioned at the end.
